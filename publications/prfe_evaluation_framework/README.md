...
In this document we present a framework for evaluating privacy-preserving reach
...
To understand the level of privacy guarantee the candidate estimators will
...
A rather complex privacy consideration occurs when generating multiple reports
...
3.  **Any Distribution Bloom Filters and Counting Bloom Filters:** Standard
...
Evaluation Criteria                                             | Secure Bloom Filter with local noise (DP) | VoC with local noise (DP)                                                         | Any Distribution [Counting] Bloom Filters
--------------------------------------------------------------- | ----------------------------------------- | --------------------------------------------------------------------------------- | -----------------------------------------
Scalability with increasing # of publishers (in big-O notation) | O(n)<sup>3</sup>                         | O(n) (Notable bias for some cases (e.g. ids if reached are only reached by 2pubs) | O(n)
Scalability of Increasing Abs Set Sizes (in big-O notation)     | O(n)                                      | Constant (4096)<sup>4</sup>                                                       | O(log(n))<sup>5</sup>
Private Input                                                   | Yes                                       | Yes                                                                               | Yes
Implementation Complexity                                       | Low                                       | Low                                                                               | High for MPC, Low for Local DP
DP Output                                                       | Yes                                       | Yes                                                                               | Yes
Estimating Frequency                                            | Approximation<sup>1</sup>                 | Approximation                                                                     | Yes for MPC-based, Maybe for non-MPC-based
Share Secret                                                    | Yes<sup>2</sup>                           | Yes                                                                               | No for MPC-based, Yes for non-MPC-based
Lossless Union                                                  | Yes                                       | No                                                                                | Yes
Scalability with increasing cardinality ratio                   | Yes                                       | TBD                                                                               | Yes
Scalability with increasing overlap ratio                       | Yes                                       | Yes                                                                               | Yes
Accuracy with DP                                                | TBD                                       | TBD                                                                               | Good for MPC-based, Non-MPC-based: TBD
...
As an initial approach, we believe simulated data is sufficient to evaluate the
...
Furthermore, in cross-media campaigns, a user’s reach probability for one
...
We expect implementation to proceed as follows: * Reach evaluation framework *
...
To model the user reach probability distribution, we use Dirac Mixture with 4
